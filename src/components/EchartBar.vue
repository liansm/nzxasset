...
export default {
  ...
  mounted () {
    this.$http.get('http://192.168.182.128/assetdb/pub/perf/get_perf_detail.php?request_param=FrameTime,GameThreadTime,RenderThreadTime,RHIThreadTime,GPUTime&&run_id=4').then((response) => {
      console.info(response.body.MapID)
      this.map_id = response.body.MapID

      this.frameTimes = response.body.FrameTime.map(Number)
      this.gameThreadTimes = response.body.GameThreadTime.map(Number)
      this.renderThreadTimes = response.body.RenderThreadTime.map(Number)
      this.rhiThreadTimes = response.body.RenderThreadTime.map(Number)
      this.gpuTimes = response.body.GPUTime.map(Number)
